 ...
 * Link: https://github.com/Skytils/SkytilsMod/blob/1.x/src/main/kotlin/skytils/skytilsmod/utils/RenderUtil.kt
 *
...
object RenderUtils {
    private val RARITY = ResourceLocation("asterium", "gui/rarity.png")
    private val RARITY2 = ResourceLocation("asterium", "gui/rarity2.png")
    private val RARITY3 = ResourceLocation("asterium", "gui/rarity3.png")
    private val RARITY4 = ResourceLocation("asterium", "gui/rarity4.png")
    private val CUSTOM_RARITY = ResourceLocation("asterium", "gui/custom_rarity.png")
    ...
    @JvmStatic
    fun renderRarity(itemStack: ItemStack?, xPos: Int, yPos: Int, alpha: Float, shape: Int) {
        if (itemStack != null) {
            if (!mutex.isLocked) {
                mutex.lock()
                renderRarity(xPos, yPos, itemStack, alpha, shape)
                mutex.unlock()
            }
        }
    }
     ...
     */
    @JvmStatic
    fun renderRarity(xPos: Int, yPos: Int, rarity: SkyblockItemRarity, alpha: Float, shape: Int) {
    ...
    private fun renderRarity(xPos: Int, yPos: Int, itemStack: ItemStack?, alpha: Float, shape: Int) {
        if (itemStack == null) return
        val rarity = SkyblockItemUtils.getRarity(itemStack)
        if (rarity != SkyblockItemRarity.NONE) {

            if (shape < 5) {
                renderRarity(xPos, yPos, rarity, alpha, shape)
            } else {
                GlStateManager.pushMatrix()
                // save the states
                val lightingEnabled = GL11.glIsEnabled(GL11.GL_LIGHTING)
